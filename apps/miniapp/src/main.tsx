...
function TaskComposer({ user, onClose }: { user: User; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: TaskForm) => {
      ...
      const response = await fetch(`${API}/tasks?actor_id=${user.id}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ title: data.title, deadline: new Date(data.deadline).toISOString(), kind: data.kind, leader_id: data.leaderId || null, member_ids: members }) })
...
function App() {
  ...
  const tasks = useQuery({ queryKey: ['tasks', auth.data?.id], queryFn: () => fetchTasks(auth.data!.id), enabled: Boolean(auth.data?.id) })
