...
function AdminSheet({ token, close }: { token: string; close: () => void }) {
  const users = useQuery({ queryKey: ['admin-users'], queryFn: () => api<AdminUser[]>('/admin/users', token) })
  const update = useMutation({ mutationFn: ({ id, patch }: { id: string; patch: object }) => api(`/admin/users/${id}`, token, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) }), onSuccess: () => client.invalidateQueries({ queryKey: ['admin-users'] }) })
  return <section className="task-sheet"><div className="composer__heading"><div><span className="eyebrow">Administration</span><h2>People</h2></div><button className="icon-button" onClick={close} aria-label="Close administration">×</button></div>{users.isLoading && <p>Loading people…</p>}{users.error && <p className="form-error">{users.error.message}</p>}{users.data?.map(person => <div className="admin-user" key={person.id}><div><strong>{person.full_name || 'Unfinished profile'}</strong><span>{person.telegram_username ? `@${person.telegram_username}` : 'No username'} · {person.status}</span></div><select value={person.role} onChange={event => update.mutate({ id: person.id, patch: { role: event.target.value } })}><option value="participant">Participant</option><option value="sector_head">Sector head</option><option value="admin">Administrator</option></select><button className="secondary" disabled={person.status === 'inactive' || update.isPending} onClick={() => update.mutate({ id: person.id, patch: { status: 'inactive' } })}>Deactivate</button></div>)}{update.error && <p className="form-error">{update.error.message}</p>}</section>
}
...
function App() {
  ...
  return <main className="shell"><header><div className="brand"><span className="mark">SS</span><div><span className="eyebrow">Operations field board</span><h1>Today’s work</h1></div></div><div className="profile"><strong>{user.full_name || 'Finish profile'}</strong><span>{user.telegram_username ? `@${user.telegram_username}` : 'Telegram user'}</span></div></header><section className="summary"><div><span>Assigned</span><strong>{tasks.data?.length ?? 0}</strong></div><div><span>Open now</span><strong>{open}</strong></div><div><span>Role</span><strong>{user.role.replace('_', ' ')}</strong></div></section>{user.role !== 'participant' && <button className="new-task" onClick={() => setComposing(true)}><span>+</span> Assign work</button>}{user.role === 'admin' && <button className="admin-button" onClick={() => setAdminOpen(true)}>Manage people</button>}<section className="task-list"><div className="section-heading"><span className="eyebrow">Your queue</span><h2>Tasks</h2></div>{tasks.isLoading && <p>Loading tasks…</p>}{tasks.error && <p className="form-error">Tasks could not be loaded.</p>}{tasks.data?.length === 0 && <div className="empty"><span>◌</span><h3>Nothing assigned yet</h3><p>New work will appear here as soon as it is assigned.</p></div>}{tasks.data?.map(task => <button className="task-card" key={task.id} onClick={() => setSelected(task)}><div><Status value={task.status} /><h3>{task.title}</h3><p>{task.description || 'No description provided.'}</p></div><time>{due(task.deadline)}</time></button>)}</section>{user.role !== 'participant' && <section className="event-list"><div className="section-heading"><span className="eyebrow">Management</span><h2>Event archives</h2></div>{events.data?.map(event => <button className="event-card" key={event.id} onClick={() => setSelectedEvent(event)}><strong>{event.title}</strong><span>{due(event.starts_at)}</span></button>)}</section>}{(selected || composing || selectedEvent || adminOpen) && <div className="sheet-backdrop">{composing ? <TaskComposer user={user} token={token} close={() => setComposing(false)} /> : selectedEvent ? <ArchiveSheet event={selectedEvent} token={token} close={() => setSelectedEvent(null)} /> : adminOpen ? <AdminSheet token={token} close={() => setAdminOpen(false)} /> : <TaskSheet task={selected!} user={user} token={token} close={() => setSelected(null)} />}</div>}</main>
}
