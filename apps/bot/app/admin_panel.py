...
router = Router(name="bot_admin_panel")
...
async def administration_choose_user_sector(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    rows.append([InlineKeyboardButton(text="🚫 Без сектора", callback_data="asetsec:none")])
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admuser:{user.id}")])
    await callback.message.edit_text(
...
async def administration_set_user_sector(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not raw_user_id:
        await callback.answer("Сессия выбора сектора истекла. Откройте карточку заново.", show_alert=True)
        return
    ...
    try:
        ...
        callback.data = f"admuser:{user_id}"
        await administration_user_card(callback)
    except Exception as exc:
        await answer_http_error(callback, exc)


def sector_card_keyboard(sector: Sector) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
...
async def administration_sectors(callback: CallbackQuery) -> None:
    ...
    rows.append([InlineKeyboardButton(text="➕ Создать сектор", callback_data="secnew")])
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:home")])
    await callback.message.edit_text(
        "🏢 Сектора",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
...
async def administration_sector_card(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        sector = await session.get(Sector, sector_id)
        user_count = len(
            list((await session.scalars(select(User.id).where(User.sector_id == sector_id))).all())
        )
    if not sector:
    ...
    await callback.message.edit_text(text, reply_markup=sector_card_keyboard(sector))
    await callback.answer()
...
async def administration_sector_create_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer("Введите название нового сектора.")
    await callback.answer()
...
async def administration_sector_rename_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer("Введите новое название сектора.")
    await callback.answer()
...
async def administration_sector_description_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(sector_edit_id=callback.data.split(":", 1)[1])
    await callback.message.answer("Введите новое описание или «-», чтобы очистить его.")
    await callback.answer()
...
async def administration_sector_state(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        callback.data = f"sector:{sector.id}"
        await administration_sector_card(callback)
    except Exception as exc:
        await answer_http_error(callback, exc)
