...
logger = logging.getLogger("ss_bot.telegram")
...
class AdminUserSector(StatesGroup):
    choose = State()
...
async def issue_task_start(message: Message, state: FSMContext) -> None:
    ...
    if actor.role not in {Role.ADMIN, Role.SECTOR_HEAD}:
        ...
        return
    await state.clear()
    await state.update_data(creator_id=str(actor.id))
...
async def finish_task_people(
    ...
    if creator_id and not data.get("creator_id"):
    ...
    if data["kind"] == "group":
        ...
        await message.answer(
            ...
            reply_markup=await leader_keyboard(data["member_ids"], data["creator_id"]),
        )
    else:
...
async def issue_task_people(message: Message, state: FSMContext) -> None:
    ...
    if query == "-":
        await finish_task_people(message, state, str(actor.id))
        return
...
async def leader_keyboard(member_ids: list[str], creator_id: str) -> InlineKeyboardMarkup:
    ids = list(dict.fromkeys([uuid.UUID(creator_id), *(uuid.UUID(value) for value in member_ids)]))
    async with SessionLocal() as session:
...
async def issue_task_people_done(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    actor = await sync_callback_user(callback)
    await finish_task_people(callback.message, state, str(actor.id))
    await callback.answer()
...
async def issue_task_leader(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    eligible_leaders = set(data.get("member_ids", [])) | {data.get("creator_id")}
    if leader_id not in eligible_leaders:
        await callback.answer("Руководитель больше не входит в состав задачи.", show_alert=True)
        return
    ...
    try:
        ...
        async with SessionLocal() as session:
            ...
            try:
                ...
                for object_key in (preview_key, original_key):
                    try:
                        delete_object(object_key)
                    except Exception:
                        logger.exception("Could not roll back uploaded report object %s", object_key)
                raise
        await state.clear()
...
async def admin_user_detail(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        await callback.answer("Некорректный пользователь.", show_alert=True)
        return
    card = await build_admin_user_card(user_id, page)
    if not card:
...
async def admin_user_role(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        card = await build_admin_user_card(user_id)
        assert card
        await callback.message.edit_text(card[0], reply_markup=card[1])
...
async def admin_user_sector_start(callback: CallbackQuery, state: FSMContext) -> None:
    actor = await sync_callback_user(callback)
    if actor.role != Role.ADMIN:
        await callback.answer("Недостаточно прав.", show_alert=True)
        return
    ...
    async with SessionLocal() as session:
        ...
        sectors = list(
            (
                await session.scalars(
                    select(Sector).where(Sector.is_active).order_by(Sector.name)
                )
            ).all()
        )
    if not target:
        await callback.answer("Пользователь не найден.", show_alert=True)
        return
    await state.set_state(AdminUserSector.choose)
    await state.update_data(admin_sector_user_id=str(user_id))
    rows = [
        [InlineKeyboardButton(text="Без сектора", callback_data="us:none")],
        *[
            [InlineKeyboardButton(text=sector.name[:52], callback_data=f"us:{sector.id}")]
            for sector in sectors
        ],
    ]
    await callback.message.edit_text(
        f"Выберите сектор для {display_user(target)}.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
    )
    await callback.answer()


@router.callback_query(AdminUserSector.choose, F.data.startswith("us:"))
async def admin_user_sector_finish(callback: CallbackQuery, state: FSMContext) -> None:
    actor = await sync_callback_user(callback)
    if actor.role != Role.ADMIN:
        await callback.answer("Недостаточно прав.", show_alert=True)
        return
    data = await state.get_data()
    try:
        user_id = uuid.UUID(data["admin_sector_user_id"])
        raw_sector_id = callback.data.split(":", 1)[1]
        sector_id = None if raw_sector_id == "none" else uuid.UUID(raw_sector_id)
    except (KeyError, ValueError):
        await callback.answer("Выбор устарел. Откройте карточку снова.", show_alert=True)
        await state.clear()
        return
    try:
        async with SessionLocal() as session:
        ...
        assert card
        await callback.message.edit_text(card[0], reply_markup=card[1])
        await callback.answer("Сектор обновлён")
    except Exception as exc:
        await answer_http_error(callback, exc)
